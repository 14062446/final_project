...
path = "Citibike Data/"
df = pd.DataFrame(pd.read_csv("Citibike Data/2013-07 - Citi Bike trip data.csv"))
...
def generate_matrix(sign):
    '''This function generates the matrices described above, 
    given the input 'plus' or 'minus' for postive change in bikes 
    over each hour and negative change in bikes, respectively. 
    First, we create an empty dataframe with the Citibike station IDs 
    as columns, and an index of the hour by hour timestamp for 2013-2015'''
    df2 = pd.DataFrame(index=datelist, columns = station_id_list)
    df2 = df2.fillna(0)
    #insert values into plus matrix
    if sign == 'plus':
    #if we are generating the plus matrix, we consider all 
    #of the end station data, as in this case a bike is going to 
    #be returning to a station, hence +1.
    ...
    #in the other case, we only consider the start station data, 
    #as a bike will be leaving a station, hence -1.    
...
def save_matrices():
    '''This function saves the plus and minus matrices so we can 
    access them later on without generating them again. We also 
    compute a net change matrix with is essentially the sum of 
    the plus and minus matrices'''
