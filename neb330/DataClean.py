...
def date_split(col_name, data_frame):
    '''This function takes in a time column of a dataframe, 
    and that dataframe, and splits the time column into the month, 
    year, hour, day, and day of the week. It appends all of these 
    values to different lists, and returns the lists.'''
    ...
    for ln in data_frame[col_name]:
        lst = ln.split(' ')
        #some of the values in the time column have different 
        #formats, so we need to try them all
        try:
...
def add_columns(data_frame):
    '''This function adds the month, year, hour, day, 
    and day of the week lists that are returned by date_split 
    to the dataframe that is input.'''
